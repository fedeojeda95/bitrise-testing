...
open class UIImageViewAligned: UIImageView {
    ...
    /**
     ...
     It is available in Interface Builder and should not be set programmatically.
     Use `alignment` property if you want to set alignment outside Interface Builder.
     */
    ...
    private func setup(image: UIImage? = nil, highlightedImage: UIImage? = nil) {
        realImageView = UIImageView(
            image: image ?? super.image,
            highlightedImage: highlightedImage ?? super.highlightedImage)
    ...
    private func updateLayout() {
        ...
        // Make sure we clear the contents of this container layer,
        // since it refreshes from the image property once in a while.
        layer.contents = nil
        if #available(iOS 11, *) {
            super.image = nil
        }
